...
Auto-start is delayed until `OBS_FRONTEND_EVENT_FINISHED_LOADING` (queued on the UI thread) so canvas plugins can finish `obs_canvas_reset_video` first.

Spout Capture sources no longer treat extra-canvas redraws as a sender reset, and they keep GPU textures until the source is hidden on every canvas.
